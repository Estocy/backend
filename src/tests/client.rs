...
use crate::{get_rocket_instance, models::client::{Client, NewClient}, models::user::{User, NewUser}};
...
fn create() {
    ...
    client.delete(format!("/clients/{}/{}",user_id,response_client.id))
...
fn show() {
    ...
    let mut response = client.get(format!("/clients/{}/{}",user_id,response_client_create.id))
    .header(ContentType::JSON)
    .dispatch();
...
fn delete() {
    ...
    let response = client.delete(format!("/clients/{}/{}",user_id,response_client.id))
        .header(ContentType::JSON)
        .dispatch();
...
fn create_client() -> (Client, Client, Uuid){
    ...
    let mut response = client.get(format!("/clients/{}/{}",response_user_create.id,response_client_create.id))
        .header(ContentType::JSON)
        .body(serde_json::to_string(&clients).unwrap())
        .dispatch();
    ...
    return (response_client, response_client_expected,response_user_create.id);
}