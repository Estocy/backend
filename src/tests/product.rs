...
use crate::{get_rocket_instance, models::product::{Product, NewProduct, ProductReceiver}, models::category::{Category, NewCategory}};
#[test]
fn create() {
...
fn show() {
    ...
    let response_create_product: Product = serde_json::from_str(response_create.body_string().unwrap().as_str()).unwrap();
    
    let mut response = client.get(format!("/products/{}",response_create_product.id))
        .header(ContentType::JSON)
        .dispatch();
    ...
    assert_eq!(response_product.0.additional_charge, expected_product.additional_charge);
    ...
    assert_eq!(response_product.0.stock_amount, expected_product.stock_amount);
    ...
    let mut response = client.delete(format!("/products/{}",response_product.0.id))
        .header(ContentType::JSON)
        .body(serde_json::to_string(&product_category).unwrap())
        .dispatch();
}
...
fn delete() {
    ...
    let response = client.delete(format!("/products/{}",response_product.id))
        .header(ContentType::JSON)
        .body(serde_json::to_string(&product_category).unwrap())
        .dispatch();
